...
/// Advance the tick counter by 1ms. Called from the SysTick interrupt handler.
pub fn tick(_: *Application) void {
    timer_module.incrementCurrentTime(1);
}

/// Run pending timer callbacks. Returns true if work was done.
pub fn run(_: *Application) bool {
    return timer_module.run();
}
