...
//! then register `onSystemReady` for post-SDK-init work like WiFi.
...
fn onSystemReady() callconv(sdk.cc) void {
    wifi.init();
...
export fn user_rf_cal_sector_set() u32 { // zlinter-disable-current-line function_naming
    return 0x3FB;
}

export fn user_init() void { // zlinter-disable-current-line function_naming
    hardware.init();
    uart.puts("Hardware initialized\r\n");
    sdk.system_init_done_cb(onSystemReady);
}
