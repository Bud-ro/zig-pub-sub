//! Application-level wiring for the STM32F407 firmware.
//! Binds ERD definitions to concrete data components, sets up the erd_core
//! TimerModule, and drives a 500ms LED blink + 1s uptime counter via the
//! Cortex-M4 SysTick super-loop.

const erd_core = @import("erd_core");
const hardware = @import("hardware.zig");
const system_erds = @import("system_erds.zig");

/// Concrete RAM data component type for STM32F407.
pub const RamDataComponent = erd_core.data_component.Ram(&system_erds.ram_definitions);

/// Aggregate of all data components for STM32F407.
pub const Components = struct {
    ram: RamDataComponent,
};

/// Fully instantiated SystemData type for STM32F407.
pub const SystemData = erd_core.SystemData(system_erds.ErdDefinitions, system_erds.ErdEnum, system_erds.erd, Components);

/// Top-level STM32F407 application state with timers and system data.
...
/// Shared timer module for the super-loop scheduler.
