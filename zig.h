...
#if _MSC_VER
#define zig_import(Type, fn_name, libc_name, sig_args, call_args) \
    zig_extern Type fn_name sig_args;                             \
...
#define zig_import_builtin(Type, fn_name, libc_name, sig_args, call_args) \
    zig_extern Type libc_name sig_args;                                   \
...
#else
#define zig_trap() __asm__ volatile("nop")
#endif
...
#define zig_int_helpers(w)                                                                                                  \
    ...
    {                                                                                                                       \
        return (val & UINT##w##_C(1) << (bits - UINT8_C(1))) != 0                                                           \
                   ? val | zig_minInt_i(w, bits)                                                                            \
                   : val & zig_maxInt_i(w, bits);                                                                           \
    }                                                                                                                       \
                                                                                                                            \
...
#define zig_int_builtins(w)                                                                        \
    ...
    {                                                                                              \
        uint##w##_t res;                                                                           \
        if (rhs >= bits)                                                                           \
            return lhs != UINT##w##_C(0) ? zig_maxInt_u(w, bits) : lhs;                            \
    ...
    {                                                                                              \
        int##w##_t res;                                                                            \
        if ((uint##w##_t)rhs < (uint##w##_t)bits && !zig_shlo_i##w(&res, lhs, (uint8_t)rhs, bits)) \
            return res;                                                                            \
    ...
    {                                                                                              \
        int##w##_t res;                                                                            \
        if (!zig_addo_i##w(&res, lhs, rhs, bits))                                                  \
            return res;                                                                            \
    ...
    {                                                                                              \
        int##w##_t res;                                                                            \
        if (!zig_subo_i##w(&res, lhs, rhs, bits))                                                  \
            return res;                                                                            \
        return res >= INT##w##_C(0) ? zig_minInt_i(w, bits) : zig_maxInt_i(w, bits);               \
    }                                                                                              \
                                                                                                   \
    ...
    {                                                                                              \
        int##w##_t res;                                                                            \
        if (!zig_mulo_i##w(&res, lhs, rhs, bits))                                                  \
            return res;                                                                            \
...
typedef struct
{
    ...
    int64_t hi;
                                                ...
                                                typedef struct
{
    zig_align(16) uint64_t hi;
    uint64_t lo;
} zig_u128;
typedef struct
{
    zig_align(16) int64_t hi;
    uint64_t lo;
} zig_i128;
#endif
...
static inline zig_u128 zig_shr_u128(zig_u128 lhs, uint8_t rhs)
{
    if (rhs == UINT8_C(0))
        return lhs;
    if (rhs >= UINT8_C(64))
        return (zig_u128){.hi = zig_minInt_u64, .lo = lhs.hi >> (rhs - UINT8_C(64))};
...
static inline zig_u128 zig_shl_u128(zig_u128 lhs, uint8_t rhs)
{
    if (rhs == UINT8_C(0))
        return lhs;
    if (rhs >= UINT8_C(64))
        return (zig_u128){.hi = lhs.lo << (rhs - UINT8_C(64)), .lo = zig_minInt_u64};
...
static inline zig_i128 zig_shr_i128(zig_i128 lhs, uint8_t rhs)
{
    if (rhs == UINT8_C(0))
        return lhs;
    if (rhs >= UINT8_C(64))
        return (zig_i128){.hi = zig_shr_i64(lhs.hi, 63), .lo = zig_shr_i64(lhs.hi, (rhs - UINT8_C(64)))};
...
static inline zig_i128 zig_shl_i128(zig_i128 lhs, uint8_t rhs)
{
    if (rhs == UINT8_C(0))
        return lhs;
    if (rhs >= UINT8_C(64))
        return (zig_i128){.hi = lhs.lo << (rhs - UINT8_C(64)), .lo = zig_minInt_u64};
...
{
    zig_i128 res;
    if (zig_cmp_u128(zig_bitCast_u128(rhs), zig_make_u128(0, bits)) < INT32_C(0) && !zig_shlo_i128(&res, lhs, (uint8_t)zig_lo_i128(rhs), bits))
        return res;
...
{
    zig_i128 res;
    if (!zig_addo_i128(&res, lhs, rhs, bits))
        return res;
...
{
    zig_i128 res;
    if (!zig_subo_i128(&res, lhs, rhs, bits))
        return res;
    return zig_cmp_i128(res, zig_make_i128(0, 0)) >= INT32_C(0) ? zig_minInt_i(128, bits) : zig_maxInt_i(128, bits);
}
...
{
    zig_i128 res;
    if (!zig_mulo_i128(&res, lhs, rhs, bits))
        return res;
...
{
    if (bits <= UINT8_C(64))
        return zig_clz_u64(zig_lo_u128(val), bits);
    if (zig_hi_u128(val) != 0)
        return zig_clz_u64(zig_hi_u128(val), bits - UINT8_C(64));
...
{
    if (zig_lo_u128(val) != 0)
        return zig_ctz_u64(zig_lo_u128(val), UINT8_C(64));
    ...
    while (remaining_bytes >= 64 / CHAR_BIT)
    {
        ...
        if (do_signed)
        {
            ...
            if (lhs_limb != rhs_limb)
                return (lhs_limb > rhs_limb) - (lhs_limb < rhs_limb);
            do_signed = false;
        }
        else
        {
...
#define zig_make_f128_zig_make_u128(hi, lo) \
    (zig_f128) { lo, hi }
...
#define zig_convert_builtin(ExternResType, ResType, operation, ExternArgType, ArgType, version)                       \
    zig_extern ExternResType zig_expand_concat(zig_expand_concat(zig_expand_concat(__##operation,                     \
                                                                 ...
                                                                 zig_compiler_rt_abbrev_##ResType),                   \
                                               version)(ExternArgType);                                               \
    static inline ResType zig_expand_concat(zig_expand_concat(zig_##operation,                                        \
                                                              zig_compiler_rt_abbrev_##ArgType),                      \
                                            zig_compiler_rt_abbrev_##ResType)(ArgType arg)                            \
    {                                                                                                                 \
        ResType res;                                                                                                  \
...
#define zig_float_less_builtin_0(Type, operation)                                                                   \
    zig_extern int32_t zig_expand_concat(zig_expand_concat(__##operation,                                           \
                                                           zig_compiler_rt_abbrev_zig_##Type),                      \
                                         2)(zig_##Type, zig_##Type);                                                \
...
#define zig_atomicrmw_xchg_float(res, obj, arg, order, Type, ReprType)                                               \
    do                                                                                                               \
    {                                                                                                                \
...
#define zig_atomicrmw_add_float(res, obj, arg, order, Type, ReprType)                                                                     \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
...
#define zig_atomicrmw_sub_float(res, obj, arg, order, Type, ReprType)                                                                     \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_min_float(res, obj, arg, order, Type, ReprType)                                                                     \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_max_float(res, obj, arg, order, Type, ReprType)                                                                     \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_xchg_int128(res, obj, arg, order, Type, ReprType)                                              \
    do                                                                                                               \
    {                                                                                                                \
        zig_##Type zig_atomicrmw_expected;                                                                           \
        zig_atomic_load(zig_atomicrmw_expected, obj, zig_memory_order_relaxed, Type, ReprType);                      \
...
#define zig_atomicrmw_add_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_sub_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_and_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_nand_int128(res, obj, arg, order, Type, ReprType)                                                                   \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_or_int128(res, obj, arg, order, Type, ReprType)                                                                     \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_xor_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_min_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
...
#define zig_atomicrmw_max_int128(res, obj, arg, order, Type, ReprType)                                                                    \
    do                                                                                                                                    \
    {                                                                                                                                     \
        zig_##Type zig_atomicrmw_expected;                                                                                                \
        zig_##Type zig_atomicrmw_desired;                                                                                                 \
